...
print("Sort Number is"+str(def_SortNum))
filecontent=['begin']
charstring=''
while((len(k.get('msg'))==10 or len(k.get('msg'))==20 )and (len(filecontent)!=0) and(Begin<End)):
    ...
    save=charstring
file=open("c:/1/"+def_Name+'('+str(def_SortNum)+')'+'.txt','wb')
file.write(save.encode('gb18030'))
file.close()
print(ID+" 's Followee Part"+str(def_SortNum)+"has been read")
